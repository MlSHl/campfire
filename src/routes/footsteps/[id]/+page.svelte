    ...
    <MobileBar isMobile={true} onBackClick={saveStep} backDestination={"/footsteps"} onButtonClick={saveStep} buttonText={"Save"} />
		<div class="mx-auto max-w-2xl px-4 pt-5">
				<input
                    ...
                    class="w-full bg-transparent text-center text-2xl font-semibold tracking-tight text-zinc-900 placeholder:text-zinc-400 focus:outline-none dark:text-zinc-100 dark:placeholder:text-zinc-500 md:text-4xl border-b border-transparent focus:border-orange-200/80 focus:dark:border-stone-800"
				/>
			...
			<div class="mt-2 overflow-hidden rounded-2xl border border-orange-200/80 bg-white/50 shadow-sm dark:border-stone-800 dark:bg-[#151a28]/60">
				<div class="p-4">
					<textarea
						...
						class="min-h-[55vh] w-full resize-none bg-transparent text-lg leading-7 text-zinc-800 placeholder:text-zinc-400 focus:outline-none dark:text-zinc-200 dark:placeholder:text-zinc-500"
					></textarea>
				</div>

				<button
					onclick={deleteStep}
					class="block w-full border-t border-orange-200 bg-orange-50/60 px-4 py-3 text-lg font-medium text-red-600 hover:bg-red-500/10 dark:border-stone-800 dark:bg-[#151a28] dark:text-red-400 dark:hover:bg-red-500/10"
				>
					delete
				</button>
			</div>
